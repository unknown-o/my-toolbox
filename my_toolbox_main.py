...
class my_toolbox_main:
    ...
    def start_scan(self,args):
        PID=str(os.popen("echo `ps ax | grep -i '/plugin/my_toolbox/nmap.py' | sed 's/^\([0-9]\{1,\}\).*/\1/g' | head -n 1`").read())
        ...
        os.popen('python3 /www/server/panel/plugin/my_toolbox/nmap.py '+port_start+' '+port_end+' '+args.ip+' &')
    ...
    def get_scan_result(self,args):
        ...
        if(os.path.exists("/www/server/panel/plugin/my_toolbox/result.tmp")):
            ...
            while line:   
                if('open' in line):
                    port_list=port_list+'<tr><td>'+line.split()[0]+'</td><td>'+line.split()[1]+'</td><td>'+line.split()[2]+'</td></tr>'
                line = file.readline() 
            file.close()
    ...
    def sitemap_made(self,args):
        ...
        if(os.path.exists("/www/server/panel/pyenv/bin/python")):
            result=os.popen('/www/server/panel/pyenv/bin/python /www/server/panel/plugin/my_toolbox/sitemap.py '+args.url).read()
        else:
            if(os.popen('command -v python3').read()==''):
                return 'Python3环境缺失！请手动安装！'
            else:
                result=os.popen('python3 /www/server/panel/plugin/my_toolbox/sitemap.py '+args.url).read()
        if(result==''):
            if(os.path.exists("/www/server/panel/plugin/my_toolbox/static/sitemap.xml")):
                file = open("/www/server/panel/plugin/my_toolbox/static/sitemap.xml") 
                while 1:
                    line = file.readline()
                    if not line:
                        break
